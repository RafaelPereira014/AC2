...
volatile int volt_max = 0x00;
volatile int volt_min = 0x33;
volatile int flag = 0;
...
void _int_(32) isr_uart2(void)
{
    unsigned char val;
    char leitura;    
    if(IFS1bits.U2EIF == 1){    // If U2EIF set then     
        if(U2STAbits.OERR == 1){      //    if overrun error then clear OERR flag    
            U2STAbits.OERR = 0;
        }else{
            leitura = U2RXREG;  //    else read U2RXREG to a dummy variable     
        }
        IFS1bits.U2RXIF = 0;
    } 

    if(IFS1bits.U2RXIF ==1) // If U2RXIF set then  
    {
        if (U2RXREG == 'L'){
        val = toBcd(volt_min);
        putc((val >> 4) + '0');
